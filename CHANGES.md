...
* 修复安装缺少`watchdog`库，导致服务无法启动。

## 1.7.0(2026-09-20)

* Web Runner 新增测试任务管理、用例有效性检查及 SQLite 持久化执行历史，支持查看报告、重新执行和删除记录。
* 优化 Python / YAML 用例树与任务选择交互，新增日间 / 夜间模式。
* 使用文件监听与 SSE 自动更新，恢复实时日志输出，完善静默模式与日志复制反馈。
* 修复端口占用、重复启动及 Ctrl+C 退出清理，保留当前目录直接运行 `lounger runner` 的方式。
* 补充单元测试、浏览器回归测试及 Windows CI。
