...
class WeComIntegration:
    """
    Send session summaries to WeCom.
    """

    def __init__(self, webhook_url: str, title: str = "Lounger Auto Test Summary") -> None:
        self.client = WeComWebhook(webhook_url=webhook_url)
        self.title = title
...
def register_wecom_integration(
        webhook_url: str,
        title: str = "Lounger Auto Test Summary",
) -> WeComIntegration:
    """
    Create and register a WeCom integration.
    """
    integration = WeComIntegration(webhook_url=webhook_url, title=title)
